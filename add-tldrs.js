#!/usr/bin/env node
/**
 * BAIRE TL;DR Patch Script
 * Run from repo root: node add-tldrs.js
 * Adds tldr field to all 16 blog posts after the description field.
 */
...
for (const [slug, tldr] of Object.entries(tldrs)) {
  ...
  if (/^\s+tldr:/m.test(content)) {
    // Replace existing tldr line
    content = content.replace(/^\s+tldr:.*,$/m, tldrLine);
    replaced++;
    console.log('  \u2713 ' + slug + ' (replaced existing tldr)');
  } else {
    // Find the description line and add tldr after it
    ...
    if (match) {
      ...
      errors.push('  \u274C Could not find description field in ' + slug + '.tsx');
      continue;
    }
  }
...
console.log('');
console.log('\u2705 Done. ' + updated + ' added, ' + replaced + ' replaced.');
...
console.log('');
console.log('Verify: grep -c "tldr:" lib/blog/posts/*.tsx | grep -v ":0"');
